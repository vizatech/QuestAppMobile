<template>
  <q-page id="ar-page" class="flex column">
      <div style='position: absolute; width:100%; text-align: center; z-index: 1;'>
        Lat: {{ position.coords.latitude }} Long: {{ position.coords.longitude }}
        <br/>
        H: {{ position.coords.heading }} S: {{ position.coords.speed }}
      </div>
  </q-page>
</template>
  ...
  methods: {
    getCurrentPosition() {
      Geolocation.getCurrentPosition().then(position => {
        ...
        this.lat_long = `latitude: ${this.position.coords.latitude}; longitude: ${this.position.coords.longitude};`
        this.$q.notify({
          color: 'green-4',
          textColor: 'white',
          icon: 'cloud_done',
          message: this.lat_long
        })
      });
    }
  },
